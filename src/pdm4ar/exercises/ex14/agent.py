...
class Pdm4arAgent(Agent):
    ...
    def on_episode_init(self, init_sim_obs: InitSimObservations):
        ...
        self.omega_min, self.omega_max = (
            init_sim_obs.model_params.omega_limits[0],
            init_sim_obs.model_params.omega_limits[1],
        )
    ...
    def get_commands(self, sim_obs: SimObservations) -> DiffDriveCommands:
        ...
        LOOKAHEAD_CRUISE = 0.20
        KP_ROT = 20
        last_point = self.trajectory[-1]

        radius = self.sg.wheelradius
        L = self.sg.wheelbase
        ...
        if dist_to_last < 0.2 and self.current_traj_idx > len(self.trajectory) - 2:
            return DiffDriveCommands(omega_l=0.0, omega_r=0.0)

        tx, ty = self.trajectory[self.current_traj_idx][0], self.trajectory[self.current_traj_idx][1]
        ...
        if alpha_fwd > math.pi:
            alpha_fwd -= 2.0 * math.pi
        if alpha_fwd < -math.pi:
            alpha_fwd += 2.0 * math.pi
        x_robot_frame = (tx - x) * math.cos(psi) + (ty - y) * math.sin(psi)

        # decide forward/backward
        if abs(alpha_fwd) > math.pi * 0.8 or x_robot_frame < 0:
            self.backwards = True
        else:
            self.backwards = False
        ...
        # moide selection
        if self.backwards:
            alpha = alpha_fwd + math.pi
            if alpha > math.pi:
                alpha -= 2.0 * math.pi
            if alpha < -math.pi:
                alpha += 2.0 * math.pi
            v_cmd = self.omega_min * radius
            w_cmd = KP_ROT * alpha
        else:
            alpha = alpha_fwd
            v_cmd = self.omega_max * radius
            w_cmd = KP_ROT * alpha

        # compute wheel speeds
        omega_r = v_cmd / radius + (w_cmd * L / 2.0) / radius
        omega_l = v_cmd / radius - (w_cmd * L / 2.0) / radius
